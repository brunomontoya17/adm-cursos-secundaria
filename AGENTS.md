...
- Comandos de dominio: `maestras.rs` (escuelas, materias, años, catálogos), `cursos.rs`, `alumnos.rs` (persona + `alumnos_cursos`), `horarios.rs`, `evaluaciones.rs` y `notas.rs`. Errores SQL se mapean a mensajes en español (`db::map_sql_error`).
