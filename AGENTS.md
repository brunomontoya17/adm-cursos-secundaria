...
**Tablas en `database.sql`:** catálogos (`niveles`, `turnos`, `divisiones`, `ciclos`, `anios_lectivos`, `jurisdicciones`, `tipos_*`, `estados_asistencia`); maestras (`escuelas`, `materias`, `cursos`, `alumnos`, `alumnos_cursos`, `horarios`); operativa (`eventos`, `evaluaciones`, `notas`, `observaciones`, `asistencias`). Schema **v1.3**: `alumnos` es nombre + apellido (sin DNI/contacto/nacimiento). DBs v1.1: Rust migra `niveles` + ciclos por nivel. DBs v1.2: tira columnas extra de `alumnos` conservando `id`. UI lista hasta paso 10; privacidad pasos 11–14 hechos (`docs/roadmap-privacidad.md`). El archivo SQLite runtime está cifrado (AES-GCM); hace falta clave al abrir. Icono de ventana: `src-tauri/icons/` generado del logo.
...
- Comandos de dominio: `maestras.rs` (escuelas, materias, años, catálogos), `cursos.rs`, `alumnos.rs` (persona + `alumnos_cursos`), `horarios.rs`, `evaluaciones.rs`, `notas.rs`, `asistencias.rs` y `eventos.rs`. Errores SQL se mapean a mensajes en español (`db::map_sql_error`).
